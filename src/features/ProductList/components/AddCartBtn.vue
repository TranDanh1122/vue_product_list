...
<template>
    <div class="relative">
        <div v-if="!cartItem" @click="cartStore.addToCart(product)"
            :class="`${layout == 'desktop' ? 'w-1/2' : 'w-4/5'} relative border-rose-900 border -translate-y-1/3 mx-auto bg-white flex justify-center gap-2 py-3 rounded-full cursor-pointer`">
        ...
        <div v-if="cartItem"
            :class="`${layout == 'desktop' ? 'w-1/2' : 'w-4/5'} relative -translate-y-1/3 mx-auto bg-red flex justify-between items-center py-3 rounded-full cursor-pointer px-4`">
            <button @click="cartStore.changeQty(cartItem.product.id, Number(cartItem.qty) - 1)"
                aria-label="decrement cart item"
            ...
            <button @click="cartStore.changeQty(cartItem.product.id, Number(cartItem.qty) + 1)"
                aria-label="increment cart item"
                class="size-4.5 border border-white flex justify-center items-center rounded-full">
